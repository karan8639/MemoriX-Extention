...
btnSave.addEventListener("click", async () => {
  ...
  try {
    // Send to background service worker for saving + vector queueing
    ...
    // Optimistic UI update: create snippet locally and add to list
    ...
    showToast("✓ Snippet saved + vector queued", "success");
  } catch (err) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "EMBEDDINGS_COMPUTED") {
    console.log("[Popup] Vector computation complete:", message.count, "embeddings");
    // Could refresh relate results here if panel is open
  }
});
